...
export default class AhScanReportRun extends SfCommand<AhScanReportRunResult> {
  ...
  public static readonly flags = {
    ...
    categories: Flags.string({
      ...
      description: messages.getMessage('flags.categories.description', [CATEGORIES.join(',')]),
      helpValue: '<category1,category2,...>',
      options: CATEGORIES,
      multiple: true,
    }),
    'pmd-rule-set': Flags.file({
      summary: messages.getMessage('flags.pmd-rule-set.summary'),
      description: messages.getMessage('flags.pmd-rule-set.description'),
      helpValue: '<path/to/pmd/file>',
    }),
    'eslint-rule-set': Flags.file({
      summary: messages.getMessage('flags.eslint-rule-set.summary'),
      description: messages.getMessage('flags.eslint-rule-set.description'),
      helpValue: '<path/to/eslint/file>',
    }),
    'quality-gate': Flags.string({
      char: 'q',
      summary: messages.getMessage('flags.quality-gate.summary'),
      description: messages.getMessage('flags.quality-gate.description', [
  ...
  public async run(): Promise<AhScanReportRunResult> {
    ...
    try {
      ...
      if (err?.message.includes('does not exists or not have access to it')) {
        flags['output-dir'] = PathUtils.getAbsolutePath(flags['output-dir']);
    ...
    this.saveResults(flags, scannerResult, qualityGateResult);
    if (flags.progress) {
      this.log(
  ...
  private getQualityGate(flags: Partial<AhScanReportFlags>): QualityGate {
    if (flags['quality-gate'] === 'Strict') {
      return STRICT_QUALITY_GATE;
    } else if (flags['quality-gate'] === 'High') {
      return HIHG_QUALITY_GATE;
    } else if (flags['quality-gate'] === 'Moderate') {
      return MEDIUM_QUALITY_GATE;
    } else if (flags['quality-gate'] === 'Relaxed') {
      return LOW_QUALITY_GATE;
    } else {
      return this.getCustomQualityGate(flags);
    }
  }

  // eslint-disable-next-line class-methods-use-this
  private getCustomQualityGate(flags: Partial<AhScanReportFlags>): QualityGate {
  ...
  // eslint-disable-next-line complexity, class-methods-use-this
  private async runScan(flags: Partial<AhScanReportFlags>): Promise<ScannerResult> {
    const git = new GitManager(flags.root ?? '');
    let activeBranch;
    const orgAlias = ProjectUtils.getOrgAlias(flags.root ?? '');
    try {
  ...
  private createHtmlPages(
    ...
    const oldQualityGateResult = this.getOldQualityGatesResult(flags);
    ...
    const folder = (flags['output-dir'] ?? '') + '/AuraHelperReport_' + dateStr.substring(0, dateStr.lastIndexOf('.'));
